...
export default function mockApi(tempBuilding, vizOptions) {
  ...
  for (let i = tempBuilding.floors.length - 1; i >= 0; i--) {
    ...
    for (let j = tempBuilding.floors[i].units.length - 1; j >= 0; j--) {
      ...
      for (
        let k = tempBuilding.floors[i].units[j].spaces.length - 1;
        k >= 0;
        k--
      ) {
        // for each radiator
        for (
          let l =
            tempBuilding.floors[i].units[j].spaces[k].radiators.length - 1;
          l >= 0;
          l--
        ) {
          const radiator =
            tempBuilding.floors[i].units[j].spaces[k].radiators[l];
          const radiatorStatus = assessRadiator(
            radiator,
            tempBuilding.retrieved_at
          );
          if (
            (radiatorStatus === "goodRadiator" &&
              !vizOptions.showGoodRadiators) ||
            (radiatorStatus === "coldRadiator" &&
              !vizOptions.showColdRadiators) ||
            (radiatorStatus === "offlineRadiator" &&
              !vizOptions.showOfflineRadiators) ||
            (radiatorStatus === "devicelessRadiator" &&
              !vizOptions.showDevicelessRadiators) ||
    ...
    for (let j = tempBuilding.floors[i].spaces.length - 1; j >= 0; j--) {
      for (
        let k = tempBuilding.floors[i].spaces[j].radiators.length - 1;
        k >= 0;
        k--
      ) {
